...
class Frame(NamedTuple):
    ...
    motion_vector_array: ndarray # (read here for information about what element in the array means (it is documented) https://github.com/LukasBommes/mv-extractor )
    decoded_fame: ndarray # Only included if you pass along decode frame


cap = VideoCap() #VideoCap is the class that has all the functions for extracting relevant information from videos

# skip decoding frames (gets enabled if decode frame in the extract vectors function is true)
...
def extract_vectors(url : str, decode_frames = False) -> list[Frame]:
    if decode_frames: #set decoding frame to true, if decoding is enabled
        cap.set_decode_frames(True)
    ...
    while True: #Loop through all frames in the video. (IMORTANT. if we need to work on streams, then impliment some system to make it not run forever)
        success, decoded_frame, motion_vectors, frame_type = cap.read()
        if success == False: #IF the video has ended or mve fails to extract motion vectors
            break
        frames.append(Frame(frame_type, motion_vectors, decoded_frame))
