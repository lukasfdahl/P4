...
class PositionalEncoding2D(nn.Module):
    """
    Sinusoidal 2D spatial positional encoding generated on the fly (on any resolution)
    for the runtime token grid size (h, w).
    """
    ...
    def forward(self, tokens: torch.Tensor, h: int, w: int) -> torch.Tensor:
        ...
        dim_t = 10000 ** (2 * (dim_t // 2) / half)  # [half]

        pos_h = torch.arange(h, device=device, dtype=torch.float32).unsqueeze(1) / dim_t  # [h, half]
        pos_w = torch.arange(w, device=device, dtype=torch.float32).unsqueeze(1) / dim_t  # [w, half]
        ...
        row_enc = pos_h.unsqueeze(1).expand(h, w, -1)  # [h, w, half]
        col_enc = pos_w.unsqueeze(0).expand(h, w, -1)  # [h, w, half]

        pos = torch.cat([row_enc, col_enc], dim=-1)    # [h, w, hidden_dim]
        pos = pos.reshape(1, h * w, self.hidden_dim)   # [1, h*w, hidden_dim]
...
class TemporalPositionalEncoding(nn.Module):
    """
    Sinusoidal 1D temporal positional encoding generated on the fly
    from the runtime frame index. to accept any resolution!
    """
    def __init__(self, hidden_dim: int):
        super().__init__()
        self.hidden_dim = hidden_dim
    ...
    def forward(self, tokens: torch.Tensor, frame_idx: int) -> torch.Tensor:
        ...
        # pos shape: [1, hidden_dim]

        pos[:, 0::2] = torch.sin(pos[:, 0::2])
        pos[:, 1::2] = torch.cos(pos[:, 1::2])

        temporal_enc = pos.reshape(1, 1, self.hidden_dim).to(dtype=dtype)
